...
struct TaskToolTests {
    ...
    @Test("task_read pattern returns matching lines with byte offsets and context")
    func readPatternSearch() async throws {
        let outputDir = makeTaskTempDir()
        defer { try? FileManager.default.removeItem(at: outputDir) }
        let manager = BackgroundTaskManager(outputDir: outputDir)
        let (taskId, outputFile) = await manager.spawn(
            runner: TaskNeverRunner(label: "pattern-search"),
            sessionId: "s1"
        )
        defer { Task { try? await manager.kill(taskId) } }
        let lines = [
        ...
        let firstOffset = "build started\ncompiling a.swift\n".utf8.count
        let tool = createTaskReadTool(manager: manager, sessionId: "s1")

        let result = try await tool.execute(
            "pattern-search",
            ["task_id": .string(taskId), "pattern": .string("error:")],
            nil,
            nil
        )
        let text = cursorResultTextForTaskTestResult(result)
        #expect(text.contains("2 match(es)"))
        #expect(text.contains("match at byte \(firstOffset) (line 3):"))
        #expect(text.contains("> error: first failure"))
        #expect(text.contains("compiling a.swift"))
        #expect(text.contains("note: after one"))

        guard case .object(let details) = result.details ?? .null,
              case .array(let matches) = details["matches"] ?? .null,
              case .object(let first) = matches.first ?? .null else {
            Issue.record("missing search details")
            return
        }
        #expect(details["is_regex"] == .bool(true))
        #expect(details["limit_reached"] == .bool(false))
        #expect(first["offset"] == .int(firstOffset))
        #expect(first["line_number"] == .int(3))
        #expect(first["context_before"] == .array([
            .string("build started"), .string("compiling a.swift"),
        ]))
        #expect(first["context_after"] == .array([
            .string("note: after one"), .string("note: after two"),
        ]))

        // A match offset pages the raw artifact from exactly that line.
        let page = try await manager.readOutput(
            taskId: taskId, sessionId: "s1", offset: firstOffset, limit: 32
        )
        #expect(page.text.hasPrefix("error: first failure"))
    }

    @Test("task_read pattern falls back to literal matching for invalid regex")
    func readPatternLiteralFallback() async throws {
        let outputDir = makeTaskTempDir()
        defer { try? FileManager.default.removeItem(at: outputDir) }
        let manager = BackgroundTaskManager(outputDir: outputDir)
        let (taskId, outputFile) = await manager.spawn(
            runner: TaskNeverRunner(label: "literal-fallback"),
            sessionId: "s1"
        )
        defer { Task { try? await manager.kill(taskId) } }
        try Data("ok\nerror(42) exploded\ndone\n".utf8).write(to: outputFile)
        ...
        let text = cursorResultTextForTaskTestResult(result)
        #expect(text.contains("1 match(es)"))
        #expect(text.contains("literal substring"))
        #expect(text.contains("error(42) exploded"))
        guard case .object(let details) = result.details ?? .null else {
            Issue.record("missing search details")
            return
        }
        #expect(details["is_regex"] == .bool(false))

        await #expect(throws: CodingToolError.self) {
            _ = try await tool.execute(
                "empty-pattern",
                ["task_id": .string(taskId), "pattern": .string("")],
                nil,
                nil
            )
        }
    ...
    @Test("task_read pattern caps matches and continues from the reported offset")
    func readPatternMatchCapAndContinuation() async throws {
        let outputDir = makeTaskTempDir()
        defer { try? FileManager.default.removeItem(at: outputDir) }
        let manager = BackgroundTaskManager(outputDir: outputDir)
        let (taskId, outputFile) = await manager.spawn(
            runner: TaskNeverRunner(label: "match-cap"),
            sessionId: "s1"
        )
        defer { Task { try? await manager.kill(taskId) } }
        let lines = (1...30).map { "error: failure number \($0)" }
        let content = lines.joined(separator: "\n") + "\n"
        try Data(content.utf8).write(to: outputFile)
        let capOffset = lines.prefix(20).map { $0.utf8.count + 1 }.reduce(0, +)
        let tool = createTaskReadTool(manager: manager, sessionId: "s1")
        ...
        guard case .object(let firstDetails) = first.details ?? .null,
              case .array(let firstMatches) = firstDetails["matches"] ?? .null else {
            Issue.record("missing first-page details")
            return
        }
        #expect(firstMatches.count == 20)
        #expect(firstDetails["limit_reached"] == .bool(true))
        #expect(firstDetails["next_offset"] == .int(capOffset))
        let firstText = cursorResultTextForTaskTestResult(first)
        #expect(firstText.contains("Match limit reached"))
        #expect(firstText.contains("\"offset\":\(capOffset)"))
        ...
        let second = try await tool.execute(
            ...
            [
                ...
                "offset": .int(capOffset),
            ],
            nil,
            nil
        )
        guard case .object(let secondDetails) = second.details ?? .null,
              case .array(let secondMatches) = secondDetails["matches"] ?? .null,
              case .object(let resumed) = secondMatches.first ?? .null else {
            Issue.record("missing continuation details")
            return
        }
        #expect(secondMatches.count == 10)
        #expect(secondDetails["limit_reached"] == .bool(false))
        #expect(secondDetails["next_offset"] == .int(content.utf8.count))
        #expect(resumed["offset"] == .int(capOffset))
        #expect(resumed["line"] == .string("error: failure number 21"))
    }
    ...
    func splitTaskSurfaceContract() {
        ...
        let expectedProperties: [String: Set<String>] = [
            ...
            "task_read": ["task_id", "offset", "limit", "pattern"],
